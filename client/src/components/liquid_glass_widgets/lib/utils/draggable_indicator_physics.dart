...
class DraggableIndicatorPhysics {
  ...
  /// - [stretchAlongMotion]: **AttendEase patch.** Swaps which axis squashes
  ///   and which stretches, so the shape elongates along its direction of
  ///   travel instead of across it. Defaults to `false`, i.e. the upstream
  ...
  static Matrix4 buildJellyTransform({
    ...
    // AttendEase patch: see the doc comment above.
    bool stretchAlongMotion = false,
  }) {
    ...
    final double dx = direction.dx.abs();
    final double dy = direction.dy.abs();

    // AttendEase patch: which axis squashes and which stretches.
    //
    // Upstream squashes *along* the direction of movement (0.5) and stretches
    // across it (0.3). For a pill crossing a tab bar that is backwards — a
    // moving object elongates along its path, and pinching it thin across the
    // path reads as the object receding rather than accelerating. On the
    // full-width trip the squash saturates its clamp and takes the pill to
    // 0.6x width, which is enough to look like it left and came back.
    //
    // The patched branch elongates along the path instead, and takes the
    // perpendicular scale as the reciprocal so the shape holds its area.
    // Simply swapping upstream's two coefficients was tried first and is
    // wrong for this shape: -0.5 on the height of a ~50 px pill flattens it to
    // 30 px, at which point the icon it is meant to contain stands proud of it
    // at both edges and the whole thing reads as a stretched band rather than
    // a moving pill. Conserving area is also just what squash-and-stretch is —
    // it is the constraint that keeps the deformation reading as one object.
    final double along = 1.0 + distortionFactor * 0.3;
    final double across = 1.0 / along;

    // Each axis interpolates between neutral and its factor by how much of the
    // motion lies on it, so diagonal velocities blend smoothly instead of
    // snapping between the two cases.
    final scaleX = stretchAlongMotion
        ? (1.0 + dx * (along - 1.0)) * (1.0 + dy * (across - 1.0))
        : (1.0 - dx * distortionFactor * 0.5) *
            (1.0 + dy * distortionFactor * 0.3);
    final scaleY = stretchAlongMotion
        ? (1.0 + dy * (along - 1.0)) * (1.0 + dx * (across - 1.0))
        : (1.0 - dy * distortionFactor * 0.5) *
            (1.0 + dx * distortionFactor * 0.3);
