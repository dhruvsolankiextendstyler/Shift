                    ...
                    return SpringBuilder(
                      ...
                      builder: (context, thickness, child) {
                        ...
                        final jellyTransform =
                          ...
                          maxDistortion: 0.8,
                          velocityScale: 10,
                          // AttendEase patch: must match the flag passed to
                          // every AnimatedGlassIndicator below, or the icon
                          // clip would deform differently from the pill it is
                          // supposed to be cut out of.
                          stretchAlongMotion: true,
                        );
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          ...
          // Selected layer — reveals the selected tab in its vibrant style
          // inside the pill's footprint. Below the pill there is only
          // `childUnselected`, so without this nothing is ever selected-styled
          // in `.off` mode.
          //
          // AttendEase patch: laid out at the FULL bar width and clipped to the
          // pill, rather than shrunk into one tab's slot.
          //
          // `selectedTabBuilder` returns a full-width `Row` of `tabCount`
          // `Expanded` children — see `_buildSelectedTabs` in
          // `tab_bar_bottom_layout.dart`, and see [_buildHighQualityMode] below,
          // which consumes it that way. This branch instead wrapped it in
          // `FractionallySizedBox(widthFactor: 1 / tabCount)`, squeezing all
          // three tabs into a third of the bar: three icons and three labels
          // piled on top of each other inside the pill. Nobody saw it while
          // `maskingQuality` was pinned to `.high`; it appeared the moment the
          // app started dropping to `.off` mid-slide.
          //
          // Clipping is what keeps exactly one label legible per tab. Because
          // the row is full width, every selected item lands precisely over its
          // unselected twin in `childUnselected` — same word, same place — so
          // the clip can travel with the pill without two *different* labels
          // ever overlapping. That is also what makes the `.off`/`.high` swap
          // invisible: both modes now reveal the same full-width row through the
          // same [JellyClipper] geometry, at the same `alignment` and
          // `thickness`. The jelly transform is left off here — deforming the
          // path is the cost `.off` exists to avoid, and at rest (`thickness`
          // 0) the clip is simply the tab slot, which is what keeps the
          // selected tab vibrant when no pill is drawn at all.
          if (widget.visible)
            Positioned.fill(
              child: IgnorePointer(
                child: ClipPath(
